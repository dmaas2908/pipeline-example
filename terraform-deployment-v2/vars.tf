...
// create: a hyphenated and non-hyphenated service name
// csv format is service_name, port     
locals {
  services = { for sname, port in csvdecode(file(./data/svcinfo.csv)) : sname => port }
}